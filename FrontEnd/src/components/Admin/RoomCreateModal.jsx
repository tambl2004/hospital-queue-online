...
const RoomCreateModal = ({
  ...
  const validate = () => {
    ...
    if (!formData.room_code.trim()) {
      newErrors.room_code = 'Mã phòng không được để trống';
    } else if (formData.room_code.trim().length < 2 || formData.room_code.trim().length > 50) {
      newErrors.room_code = 'Mã phòng phải từ 2 đến 50 ký tự';
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <h2 className="text-xl font-bold text-gray-800">Thêm phòng khám mới</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                errors.room_code ? 'border-red-500' : 'border-gray-300'
              }`}
              placeholder="VD: P101, K02, Tầng1-01"
              disabled={loading}
            />
            {errors.room_code && <p className="mt-1 text-sm text-red-500">{errors.room_code}</p>}
            <p className="mt-1 text-xs text-gray-500">
              Mã phòng sẽ được tự động chuyển thành chữ hoa và không được trùng
            </p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Nhập tên phòng (không bắt buộc)"
              disabled={loading}
            />
          </div>
